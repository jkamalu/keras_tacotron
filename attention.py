...
# Original code from https://gist.github.com/mbollmann/ccc735366221e4dba9f89d2aab86da1e
# TODO: Convert from input to
...
inp = np.arange(4*8).reshape(4,8)
memory = np.arange(4*5).reshape(4,5)
num_units = 10

trueInputs = Input(shape=(4,8))
memoryInputs = Input(shape=(4,5))

#attention = AttentionGRU(num_units, input_shape=(4,8))([inp, memory])
attention = AttentionGRU(num_units)([trueInputs, memoryInputs])

#model = Model(inputs=[inp, memory], outputs=attention)
